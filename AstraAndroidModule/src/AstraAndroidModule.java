...
public class AstraAndroidModule
    extends Godot.SingletonBase { // class name should be same as java file
                                  // name...duh
  ...
  public AstraAndroidModule(Activity p_activity) {
    registerClass(
        "AstraAndroidModule",
        new String[] {"myFirstFunction", "getData", "set_instance_id"});
  ...
  private void getData() {
    new Thread(new Runnable() {
      public void run() {
        ...
        /* THIS WORKS!!!
ArrayList<Vector3D> vector3DList = get3DVectors();
...
*/
  ...
  public void startBodyStream() {
    ...
    // Call depth stream once to set resoultion. Can't set resolution via point
    // stream.
    if (first_call) {
      getDepthData();
      first_call = false;
      ...
      reader.addFrameListener(new StreamReader.FrameListener() {
        public void onFrameReady(StreamReader reader, ReaderFrame frame) {
          ...
          if (bodyFrame.isValid()) {
            Log.d(logTagMy, "bodyFrame is valid: " + instanceId);

            for (Body body : bodies) {
              Dictionary joints = new Dictionary();
              for (Joint joint : body.getJoints()) {
                Log.d(logTagMy, joint.getType().name());
                ...
                Log.d(logTagMy, "( " + pos.getX() + " , " + pos.getY() + " , " +
                                    pos.getZ() + " )");
              }
              GodotLib.calldeferred(instanceId, "_new_body_frame",
                                    new Object[] {joints});
            }
        ...
        public void onFrameReady(StreamReader reader, ReaderFrame frame) {
          ...
          if (pf.isValid()) {
            ...
            while (buffer.hasRemaining()) {
              vector3DList.add(
                  new Vector3D(buffer.get(), buffer.get(), buffer.get() * -1));
            }
            frameFinished[0] = true;
          }
        }
  ...
  public ArrayList<Vector3D> getDepthData() {
    ...
    try {
      ...
      Log.e(logTagDepthStream, "thishappens");
  ...
  // protected void onMainActivityResult(int requestCode, int resultCode, Intent
  // data) {}
  //
